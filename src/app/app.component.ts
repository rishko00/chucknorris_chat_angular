...
export class AppComponent {
    ...
    dialogs: Dialog[] = [
        ...
        {user1: this.me, user2: this.users[1], 
            messages: [{time: new Date("2020-10-30").toISOString(), text: 'Privet', user_from: this.users[1]}]
            .concat(this.loadMessages(1))
        },

        {user1: this.me, user2: this.users[2], messages: [
            {text: 'Privet', user_from: this.users[2], time: new Date("2020-10-26").toISOString()},
            {text: 'Kak dela?', user_from: this.users[2], time: new Date("2020-10-27").toISOString()},
        ].concat(this.loadMessages(2))},

        {user1: this.me, user2: this.users[3], messages: [
            {text: 'Privet', user_from: this.users[3], time: new Date("2020-10-26").toISOString()},
            {text: 'Kak dela?', user_from: this.users[3], time: new Date("2020-10-28").toISOString()},
    ...
    sortedDialogs(): Dialog[] { 
        return this.dialogs.slice().sort(function(a, b) {
            ...
            ? -1 : ((a.messages[a.messages.length - 1].time < b.messages[b.messages.length - 1].time) 
            ? 1 : 0);
        })
    }
    ...
    pushCurrentDialog(user_from: User, user_to: User, text: string){
        for(let d of this.dialogs){
            if(d.user2.nickname == user_to.nickname || d.user2.nickname == user_from.nickname){
                d.messages.push({text: text, user_from: user_from, time: new Date(Date.now()).toISOString()})
                localStorage.setItem('dialogs[' + this.dialogs.indexOf(d) + '].messages', JSON.stringify(d.messages))
            }
        }
    }
